...
namespace FlightControlWeb.DB
{
    public class FlightPlanDB : IFlightPlanDB
    {
        ...
        public async Task DeleteFlight(string id)
        {
            ...
            using var cmd = new SQLiteCommand("DELETE FROM InitLocation WHERE Id = '"
                + initId + "';", con);
        ...
        private async Task<long> GetInitId(string fpId)
        {
            using SQLiteConnection con = new SQLiteConnection(_connectionString);
            await con.OpenAsync();
            using var cmd = new SQLiteCommand("SELECT * FROM FlightPlans WHERE id = '" + fpId + "';"
                , con);
        ...
        public async IAsyncEnumerable<FlightPlan> LoadAllFP()
        {
            ...
            while (await rdr.ReadAsync())
            {
                ...
                fp.Segments = JsonSerializer.Deserialize<List<FlightPlan.Segment>>(rdr.GetString(4));
                yield return fp;
            }
        }
        ...
        public async Task SaveFP(FlightPlan fp)
        {
            ...
            cmd.Parameters.AddWithValue("@InitialLocationId", await _initLocationDb.SaveInitLocation(fp.InitLocation));
